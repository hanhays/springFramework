							...
							<table>
								<thead>
									<tr>
										...
										<th class="product-name">cancel</th>
									</tr>
								</thead>
								<tbody>
									...
									<tr>
										...
										<td class="product-name">
											<c:choose>
												...
												<c:when test="${ovo.order_state == 0 && ovo.order_cancel == 1}">취소중</c:when>
												<c:when test="${ovo.order_state == 1 && ovo.order_cancel == 0}">주문완료</c:when>
												<c:when test="${ovo.order_state == 1 && ovo.order_cancel == 1}">취소완료</c:when>
											</c:choose>
										</td>
										<td class="product-name">
											<c:if test="${ovo.order_state == 0 && ovo.order_cancel == 0}">
												<!-- order_cancel = 1 로 갱신  -->
												<a href="/order/cancel/${ovo.order_idnumber}">주문취소</a>
											</c:if>
										</td>
									</tr>
									</c:forEach>
